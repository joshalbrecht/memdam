...
class CreateTest(tests.unit.server.web.FlaskResourceTestCase):
    def runTest(self):
        ...
        with self.context('/api/v1/blobs/' + blob_ref.name, method='PUT', data={'file': (StringIO.StringIO(test_data), 'test.txt')}, headers=self.headers):
            ...
            memdam.server.web.utils.get_blobstore(self.username).get_data_to_file(blob_ref, result_file)
...
class HijackingFilenameTest(tests.unit.server.web.FlaskResourceTestCase):
    ...
    def runTest(self):
        ...
        with self.context('/api/v1/blobs/' + bad_blob_id + blob_ref.extension, method='PUT', data={'file': (StringIO.StringIO(test_data), 'test.txt')}, headers=new_headers):
...
class FetchTest(tests.unit.server.web.FlaskResourceTestCase):
    def runTest(self):
        ...
        with open(some_file, 'wb') as outfile:
            outfile.write(test_data)
        with self.context('/api/v1/blobs/' + blob_ref.name, method='GET', headers=self.headers):
            memdam.server.web.utils.get_blobstore(self.username).set_data_from_file(blob_ref, some_file)
