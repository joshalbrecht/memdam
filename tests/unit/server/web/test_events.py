...
class FetchTest(tests.unit.server.web.FlaskResourceTestCase):
    def runTest(self):
        ...
        with self.context('/api/v1/events/' + event.id__id.hex, method='GET', data=event_json, headers=self.headers):
            memdam.server.web.utils.get_archive(self.username).save([event])
...
class NoAuthenticationTest(tests.unit.server.web.FlaskResourceTestCase):
    def runTest(self):
        ...
        with self.context('/api/v1/events/' + event.id__id.hex, method='GET', data=event_json, headers={}):
            result = memdam.server.web.events.events(event.id__id.hex)
            # pylint: disable=E1103
